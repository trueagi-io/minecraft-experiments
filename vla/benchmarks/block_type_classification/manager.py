...
class DatasetManager:
    ...
    def make_dataloaders(self, train_json, test_json, transform=None, batch=32, workers=2):
        train_raw = self.load_json(train_json)
        test_raw = self.load_json(test_json)

        if self.label_type == LabelType.TYPE_CLASSIFICATION:
            train_items, nc = self.flatten_class_map(train_raw)
